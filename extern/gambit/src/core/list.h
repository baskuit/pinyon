...
namespace Gambit
{
  ...
  template <class T>
  int List<T>::InsertAt(const T &t, int num)
  {
  ...
  template <class T>
  bool List<T>::operator==(const List<T> &b) const
  {
  ...
  template <class T>
  const T &List<T>::operator[](int num) const
  {
  ...
  template <class T>
  List<T> List<T>::operator+(const List<T> &b) const
  {
    List<T> result(*this);
    Node *n = b.m_head;
    while (n)
  ...
  template <class T>
  int List<T>::Insert(const T &t, int n)
  {
    return InsertAt(t, (n < 1) ? 1 : ((n > m_length + 1) ? m_length + 1 : n));
  }

  template <class T>
  T List<T>::Remove(int num)
  {
    if (num < 1 || num > m_length)
      throw IndexException();
    Node *n;
