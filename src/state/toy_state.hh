...
public:
    ...
    ToyState(prng &device) : SolvedState<size, int, int>(device, .5f, 2, 2) {} // TODO: Need to calculate payoff, strategies on initialization too
    ToyState(prng &device, ToyStatePhase phase, int pp, int length) : SolvedState<size, int, int>(device, .5f, 2, 2), phase(phase), pp(pp), length(length) {}
...
public:
    ...
    MoldState<size>(prng &device, int depth) : State<size, int, int>(device), depth(depth) {}
