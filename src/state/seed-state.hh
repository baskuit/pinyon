...
class SeedStateVector : public StateChanceVector<int, int, double>
{
public:
    ...
    int (*actions_func)(SeedStateVector *, int) = &(SeedStateVector::af);
    int (*payoff_bias_func)(SeedStateVector *, int) = &(SeedStateVector::pbf);
    ...
    // SeedStateVector is only used to generate a TreeState, and the grow algorithm only calls the other apply_actions
    void apply_actions(int row_action, int col_action)
    {
        const int transition_idx = get_transition_idx(row_action, col_action, 0);
        std::copy_n(
    ...
    */
    ...
    static int dbf(SeedStateVector *state, int depth)
    {
        return (depth - 1) * (depth >= 0);
    }
    static int af(SeedStateVector *state, int n_actions)
    {
        return n_actions;
    }
    static int pbf(SeedStateVector *state, int payoff_bias)
    {
        const int bias = 1;
