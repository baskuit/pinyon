...
namespace LRSNash
{
        ...
        long den = 100)
    {
        ...
        const long row_den = mpz_get_ui(row_solution_data[0]);
        const long col_den = mpz_get_ui(col_solution_data[0]);
        ...
        const Real row_payoff{mpz_get_ui(row_solution_data[rows + 1]) / static_cast<typename Real::type>(row_den)};
        const Real col_payoff{mpz_get_ui(col_solution_data[cols + 1]) / static_cast<typename Real::type>(col_den)};
        delete[] row_solution_data;
        delete[] col_solution_data;
