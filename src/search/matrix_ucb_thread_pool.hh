    ...
    std::mutex& mtx = mutex_pool[matrix_node->stats.mutex_idx];
        ...
        mtx.lock();
            if (matrix_node->expanded == true) {
                ...
                std::array<double, MatrixUCB::state_t::size_> strategy0 = {1/(double)matrix_node->pair.rows};
                std::array<double, MatrixUCB::state_t::size_> strategy1 = {1/(double)matrix_node->pair.cols};
                process_matrix(
