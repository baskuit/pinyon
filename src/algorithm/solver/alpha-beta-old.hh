        ...
        for (int row_idx = 0; row_idx < state.row_actions.size(); ++row_idx)
        {
            ...
            for (int j = 0; j < J.size(); ++j)
            {
                ...
                if (y > typename Types::Real{0} && p_ij < o_ij)
                {
                    ...
                    const typename Types::Real p__ij = std::max(p_ij.value, ((best_response_row - expected_o_payoff + y * o_ij) / y).value);
        ...
        typename Types::VectorReal &row_strategy)
    {
        ...
        for (int col_idx = 0; col_idx < state.col_actions.size(); ++col_idx)
        {
            ...
            for (int i = 0; i < I.size(); ++i)
            {
                ...
                if (x > typename Types::Real{0} && p_ij < o_ij)
                {
                    const typename Types::Real o__ij = std::min((o_ij).value, ((best_response_col - expected_p_payoff + x * p_ij) / x).value);
