...
{
    Atomic() : std::atomic<T>{} {}
    Atomic(T t) : std::atomic<T>{t} {}
    Atomic(const Atomic &other) : std::atomic<T>{other.load()} {}
...
template <typename T>
struct VectorAtomic : public std::vector<Atomic<T>> {
    std::atomic<size_t> _size{0};
    void resize(size_t n) {
        std::vector<Atomic<T>>::resize(n);
        _size.store(n);
    }
    void resize(size_t n, const Atomic<T> val) {
        std::vector<Atomic<T>>::resize(n, val);
        _size.store(n);
    }
    size_t size () const {
        return _size.load();
...
struct Exp3MP : Types
{
    ...
    // using real = typename Types::Real::type;
    using real = double;
    ...
    struct MatrixStats
    {
        int rows;
        int cols;
        VectorAtomic<real> row_gains;
        VectorAtomic<real> col_gains;
        VectorAtomic<int> row_visits;
        VectorAtomic<int> col_visits;
    ...
    public:
        ...
        friend std::ostream &operator<<(std::ostream &os, const Exp3MP &session)
        {
        ...
        {
            const size_t rows = stats.rows;
            const size_t cols = stats.cols;
            ...
            Types::Mutex &mutex) const
        {
            ...
            stats.row_visits[outcome.row_idx] += 1;
            stats.col_visits[outcome.col_idx] += 1;
            stats.row_gains[0].fetch_add(.1);
            if ((stats.row_gains[outcome.row_idx].fetch_add((outcome.value.get_row_value() / outcome.row_mu).value)) >= 0)
            {
                const auto max = stats.row_gains[outcome.row_idx];
                for (auto &v : stats.row_gains)
                {
            ...
            if ((stats.col_gains[outcome.col_idx].fetch_add((outcome.value.get_col_value() / outcome.col_mu).value)) >= 0)
            {
                const auto max = stats.col_gains[outcome.col_idx];
                for (auto &v : stats.col_gains)
                {
    ...
    private:
            ...
            Real eta) const
        {
            ...
            {
                const Real y{std::exp(gains[i].load() * eta.value)};
                forecast[i] = y;
                sum += y;
            }
            for (size_t i = 0; i < k; ++i)
